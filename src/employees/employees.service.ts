...
export class EmployeesService {
  ...
  async update(
    ...
    // ───────────────────────────────────────────────
    //
    // Normalize date fields to avoid Prisma DateTime parsing errors when empty strings are sent from client
    const sanitizedData: Prisma.EmployeeUpdateInput = { ...(dto as any) };

    if ((dto as any).birthdate !== undefined) {
      const v = (dto as any).birthdate;
      // Treat empty string or null as null in DB; otherwise, parse valid date
      if (!v) {
        ...
        if (isNaN(d.getTime())) {
          throw new BadRequestException(
            'Invalid birthdate. Expect ISO-8601 date string (e.g., 1990-12-31)',
          );
        }
        sanitizedData.birthdate = d;
      }
    }

    if ((dto as any).hireDate !== undefined) {
      const v = (dto as any).hireDate;
      // If empty string provided, don't overwrite existing hireDate; only set when a value is provided
      if (!v) {
        sanitizedData.hireDate = undefined;
      } else {
        const d = new Date(v);
        if (isNaN(d.getTime())) {
          throw new BadRequestException(
            'Invalid hireDate. Expect ISO-8601 date string (e.g., 2025-01-31)',
          );
        }
        sanitizedData.hireDate = d;
      }
    }
